...
- `@coldsmirk/ledger-mantine` (`packages/mantine`) — the `<DataTable>` component, the `useDataTable` hook, compound components (`DataTable.Search` / `.ColumnsMenu` / `.Pagination` / `.SelectionBar`), curated TanStack re-exports, the `zhCN` locale (`./locales`), the `toCsv` utility, and the layered `styles.css`.
