...
- **Drafts survive virtualization**: the controller owns the draft store, so an editing row that scrolls out of the virtual window and back keeps its pending values. The store belongs to exactly one row and follows the row that rendered — including when `editingRowId` is moved from outside — so editors never read a neighbour's pending values. A commit takes back exactly the values it sent once it settles, so a second `stopEditing` arriving before the row closes cannot repeat the write — while anything typed while the request was out survives it, because a custom editor is never disabled. Cancelling discards the whole pending edit and puts the mounted editors back to the row's own values.
