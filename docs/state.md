...
TanStack v9 wires features, row-model factories, and the `filterFns` / `sortFns` / `aggregationFns` registries through a required `features` option. Ledger assembles that object itself (`ledger-features.ts`): every stock feature except `columnResizingFeature` (the resize interaction is ledger's own — [sizing.md](sizing.md)) and `cellSelectionFeature` (deferred until its design round — [DESIGN.md](DESIGN.md)), all client row models (the `*Mode` translations decide at runtime whether they process rows), and **every built-in fn registered under its conventional name** — v9 resolves string ids, `'auto'` included, only against registered functions, and ledger's contract is that every capability of a raw `ColumnDef` keeps working.
