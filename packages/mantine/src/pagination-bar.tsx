...
function PaginationBarCore<TData extends RowData>({
  ...
  // from its data renders blank. Always de-duplicated and ascending, so the list a reader sees
  // does not depend on how the current size got there.
  const sizeOptions = [...new Set([...pageSizeOptions, pageSize])].toSorted((left, right) => left - right);
